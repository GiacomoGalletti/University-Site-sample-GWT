...
/**
 * componente che deve permettere di inviare i voti l'idea pi� facile per
 * implementarlo � 1 : mostrare gli esami del docente con una textarea o ListBox
 * 2 : richiedere al docente di scrivere il nome dell'esame del quale si
 * vogliono inserire i voti 3 : mostrare elenco studenti iscritti (textarea o
 * ListBox?) 4 : prendere in input il voto di ciascun utente.
 * 
 */
...
public class ProfessorSendGradesComponent extends Composite {
	...
	private void retrieveStudents(String selectedExam) {
		ConnServiceSingleton.getConnService().getStudentsExamList(selectedExam, new AsyncCallback<String>() {
	...
	private void retrieveExams() {
		...
		ConnServiceSingleton.getConnService().retrieveExams(Session.getSession().getEmail(),
	...
	private boolean fieldVerifier(String[] students, String[] grades) {
	...
	private void sendGrades() {
		...
		String selectedExam = examsList.getSelectedItemText();
		if (fieldVerifier(studentsList, gradesList)) {
			ConnServiceSingleton.getConnService().sendGrades(selectedExam, studentsList, gradesList,
			...
			Window.alert("Non c'è corrispondenza tra voti e studenti.\nInserimento non avvenuto.");
			gradesArea.setText("");
		}
	}
